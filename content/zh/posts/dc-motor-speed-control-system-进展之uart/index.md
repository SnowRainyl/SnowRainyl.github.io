...
因为没有时钟线，两端的独立时钟需要**定时同步**一下，来对齐采样点。两端的时钟理想情况下有相同的频率（提前约定好的波特率），但是由于制造的微小误差，即便双方都设定了115200的波特率，实际上也会有微小的快慢差异。因此，接收端需要一个信号来同步自己的采样节奏

那么，在RX和TX上传输的数据是什么呢？是数据帧。这个同步信号就在数据帧中。
